...
pub trait ExtractorPlugin: Send + Sync {
    ...
    fn extract(&self, db: &Database, entry_id: i64, path: &Path, ext: &str)
        -> Result<bool, String>;
}
...
impl ExtractorRegistry {
    ...
    pub fn find_plugins(&self, kind: &str, ext: &str) -> Vec<Arc<dyn ExtractorPlugin>> {
        self.plugins
            ...
            .filter(|p| {
                let kind_match =
                    p.supported_kinds().is_empty() || p.supported_kinds().contains(&kind);
                let ext_match =
                    p.supported_extensions().is_empty() || p.supported_extensions().contains(&ext);
...
impl ExtractorPlugin for BuiltinImageExtractor {
    ...
    fn extract(
        ...
        super::extract_image_meta(db, entry_id, path)
            .map(|_| true)
...
impl ExtractorPlugin for BuiltinAudioExtractor {
    ...
    fn supported_extensions(&self) -> &[&str] {
        &["mp3", "flac", "ogg", "m4a", "aac", "wma", "wav", "opus"]
    }
    fn extract(
        &self,
        db: &Database,
        ...
        super::extract_audio_meta(db, entry_id, path, ext)
            .map(|_| true)
            .map_err(|e| e.to_string())
    }
}
...
impl ExtractorPlugin for BuiltinVideoExtractor {
    ...
    fn extract(
        ...
        super::extract_video_meta(db, entry_id, path)
            .map(|_| true)
            .map_err(|e| e.to_string())
    }
}
...
impl ExtractorPlugin for BuiltinDocumentExtractor {
    ...
    fn extract(
        ...
        super::extract_document_meta(db, entry_id, path, ext)
            .map(|_| true)
            .map_err(|e| e.to_string())
    }
}
