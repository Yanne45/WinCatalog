...
pub enum Action {
    ...
    AddTag { tag_name: String, tag_color: Option<String> },
...
pub fn run_rules_post_scan(
    ...
    if enabled.is_empty() {
        return Ok(RuleStats { rules_evaluated: 0, entries_matched: 0, actions_applied: 0 });
    }
    ...
    log::info!(
        ...
        stats.rules_evaluated, stats.entries_matched, stats.actions_applied
    );
...
fn apply_action(db: &Database, entry_id: i64, action: &Action, now: i64) -> DbResult<()> {
    match action {
        Action::AddTag { tag_name, tag_color } => {
