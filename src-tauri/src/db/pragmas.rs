...
pub fn integrity_check(conn: &Connection) -> DbResult<()> {
    ...
    if result == "ok" { Ok(()) }
    else { Err(super::DbError::Migration(format!("Integrity check failed: {}", result))) }
}
...
pub fn get_diagnostics(conn: &Connection) -> DbResult<PragmaDiagnostics> {
    ...
    let fk: bool = conn.pragma_query_value(None, "foreign_keys", |r| r.get::<_, i32>(0).map(|v| v != 0))?;

    Ok(PragmaDiagnostics {
        journal_mode,
        synchronous,
        cache_size_kb: if cache_size < 0 { -cache_size } else { cache_size * page_size / 1024 },
