...
fn map_volume(row: &Row<'_>) -> rusqlite::Result<Volume> {
    Ok(Volume {
        id: row.get(0)?, label: row.get(1)?, root_path: row.get(2)?,
        fs_uuid: row.get(3)?, fs_type: row.get(4)?,
        total_bytes: row.get(5)?, free_bytes: row.get(6)?, used_bytes: row.get(7)?,
        is_online: row.get::<_, i32>(8)? != 0, last_online_at: row.get(9)?,
        auto_detect: row.get::<_, i32>(10)? != 0, scan_mode: row.get(11)?,
        last_scan_at: row.get(12)?, last_quick_scan_at: row.get(13)?,
        location_id: row.get(14)?, disk_number: row.get(15)?, created_at: row.get(16)?,
    })
}
...
fn map_entry_slim(row: &Row<'_>) -> rusqlite::Result<EntrySlim> {
    Ok(EntrySlim {
        ...
        is_dir: row.get::<_, i32>(2)? != 0, kind: row.get(3)?,
        ext: row.get(4)?, size_bytes: row.get(5)?,
        mtime: row.get(6)?, status: row.get(7)?,
    })
}
...
fn map_entry(row: &Row<'_>) -> rusqlite::Result<Entry> {
    Ok(Entry {
        id: row.get(0)?, volume_id: row.get(1)?,
        path: row.get(2)?, parent_path: row.get(3)?,
        name: row.get(4)?, is_dir: row.get::<_, i32>(5)? != 0,
        kind: row.get(6)?, ext: row.get(7)?, mime: row.get(8)?,
        size_bytes: row.get(9)?, mtime: row.get(10)?,
        ctime: row.get(11)?, atime: row.get(12)?,
        status: row.get(13)?, last_seen_at: row.get(14)?,
        quick_hash: row.get(15)?, full_hash: row.get(16)?,
    })
}

fn map_job(row: &Row<'_>) -> rusqlite::Result<Job> {
    Ok(Job {
        id: row.get(0)?, r#type: row.get(1)?, variant: row.get(2)?,
        entry_id: row.get(3)?, volume_id: row.get(4)?,
        priority: row.get(5)?, status: row.get(6)?,
        progress: row.get(7)?, attempts: row.get(8)?, last_error: row.get(9)?,
    })
}
...
pub fn get_volume(conn: &Connection, id: i64) -> DbResult<Option<Volume>> {
    Ok(conn.prepare_cached(&format!("{} WHERE id = ?1", VOLUME_SELECT))?
        .query_row(params![id], map_volume).optional()?)
}

pub fn insert_volume(conn: &Connection, label: &str, root_path: &str, fs_uuid: Option<&str>, fs_type: Option<&str>, now: i64) -> DbResult<i64> {
...
pub fn find_volume_by_uuid(conn: &Connection, fs_uuid: &str) -> DbResult<Option<Volume>> {
    Ok(conn.prepare_cached(&format!("{} WHERE fs_uuid = ?1", VOLUME_SELECT))?
        .query_row(params![fs_uuid], map_volume).optional()?)
}
...
pub fn list_entries_slim(
    conn: &Connection, volume_id: i64, parent_path: &str,
    cursor: Option<(i64, i64)>, limit: i64,
) -> DbResult<Vec<EntrySlim>> {
    if let Some((cursor_mtime, cursor_id)) = cursor {
        let mut stmt = conn.prepare_cached(
            "SELECT id,name,is_dir,kind,ext,size_bytes,mtime,status FROM entries
             WHERE volume_id=?1 AND parent_path=?2 AND status='present'
               AND (mtime,id) < (?3,?4)
             ORDER BY mtime DESC, id DESC LIMIT ?5")?;
        let rows = stmt.query_map(params![volume_id, parent_path, cursor_mtime, cursor_id, limit], map_entry_slim)?
        ...
        let mut stmt = conn.prepare_cached(
            "SELECT id,name,is_dir,kind,ext,size_bytes,mtime,status FROM entries
             WHERE volume_id=?1 AND parent_path=?2 AND status='present'
             ORDER BY is_dir DESC, mtime DESC, id DESC LIMIT ?3")?;
        let rows = stmt.query_map(params![volume_id, parent_path, limit], map_entry_slim)?
            .filter_map(|r| r.ok()).collect();
        Ok(rows)
    }
}
...
pub fn get_entry(conn: &Connection, id: i64) -> DbResult<Option<Entry>> {
    Ok(conn.prepare_cached(&format!("{} WHERE id = ?1", ENTRY_SELECT))?
        .query_row(params![id], map_entry).optional()?)
}

pub fn update_entry_hash(conn: &Connection, entry_id: i64, quick_hash: Option<&str>, full_hash: Option<&str>, algo: &str) -> DbResult<()> {
    conn.execute("UPDATE entries SET quick_hash=?1, full_hash=?2, hash_algo=?3 WHERE id=?4",
...
pub fn search_entries(conn: &Connection, query: &str, limit: i64) -> DbResult<Vec<SearchResult>> {
    ...
    let rows = stmt.query_map(params![query, limit], |row| Ok(SearchResult {
        ...
        kind: row.get(3)?, size_bytes: row.get(4)?, volume_id: row.get(5)?, rank: row.get(6)?,
    }))?.filter_map(|r| r.ok()).collect();
    Ok(rows)
}

pub fn search_text_content(conn: &Connection, query: &str, limit: i64) -> DbResult<Vec<SearchResult>> {
    let mut stmt = conn.prepare_cached(
...
pub fn dequeue_job(conn: &Connection) -> DbResult<Option<Job>> {
    Ok(conn.prepare_cached(
        ...
        .query_row([], map_job).optional()?)
}

pub fn update_job_status(conn: &Connection, job_id: i64, status: &str, progress: f64, error: Option<&str>, now: i64) -> DbResult<()> {
    conn.execute(
...
pub fn list_active_jobs(conn: &Connection) -> DbResult<Vec<Job>> {
    ...
    let rows = stmt.query_map([], map_job)?.filter_map(|r| r.ok()).collect();
    Ok(rows)
}
...
pub fn find_duplicates(conn: &Connection, min_size: Option<i64>) -> DbResult<Vec<(String, i64, i64)>> {
    let mut stmt = conn.prepare_cached(
    ...
    let rows = stmt.query_map(params![min_size.unwrap_or(0)], |r| Ok((r.get(0)?, r.get(1)?, r.get(2)?)))?
        .filter_map(|r| r.ok()).collect();
    Ok(rows)
}
...
pub fn get_duplicate_group(conn: &Connection, hash: &str) -> DbResult<Vec<Entry>> {
    let mut stmt = conn.prepare_cached(
        &format!("{} WHERE full_hash=?1 AND status='present' ORDER BY mtime DESC", ENTRY_SELECT))?;
    let rows = stmt.query_map(params![hash], map_entry)?.filter_map(|r| r.ok()).collect();
    Ok(rows)
}
...
pub fn get_setting(conn: &Connection, key: &str) -> DbResult<Option<String>> {
    Ok(conn.prepare_cached("SELECT value FROM settings WHERE key=?1")?
        .query_row(params![key], |r| r.get(0)).optional()?)
}

pub fn set_setting(conn: &Connection, key: &str, value: &str, now: i64) -> DbResult<()> {
    conn.execute("INSERT INTO settings (key,value,updated_at) VALUES (?1,?2,?3)
...
pub fn create_tag(conn: &Connection, name: &str, color: Option<&str>, now: i64) -> DbResult<i64> {
    conn.execute("INSERT INTO tags (name,color,created_at) VALUES (?1,?2,?3)", params![name, color, now])?;
    Ok(conn.last_insert_rowid())
}

pub fn tag_entry(conn: &Connection, entry_id: i64, tag_id: i64) -> DbResult<()> {
    conn.execute("INSERT OR IGNORE INTO entry_tags (entry_id,tag_id) VALUES (?1,?2)", params![entry_id, tag_id])?;
    Ok(())
}

pub fn get_entry_tags(conn: &Connection, entry_id: i64) -> DbResult<Vec<(i64, String, Option<String>)>> {
    let mut stmt = conn.prepare_cached(
        "SELECT t.id,t.name,t.color FROM tags t JOIN entry_tags et ON et.tag_id=t.id WHERE et.entry_id=?1 ORDER BY t.name")?;
    let rows = stmt.query_map(params![entry_id], |r| Ok((r.get(0)?, r.get(1)?, r.get(2)?)))?
        .filter_map(|r| r.ok()).collect();
    Ok(rows)
}
...
pub fn trash_entry(conn: &Connection, entry_id: i64, volume_id: i64, original_path: &str, original_hash: Option<&str>,
    reason: &str, size_bytes: i64, meta_snapshot: Option<&str>, now: i64, retention_days: i64) -> DbResult<i64>
{
    ...
    conn.execute("UPDATE entries SET status='deleted' WHERE id=?1", params![entry_id])?;
    Ok(conn.last_insert_rowid())
}
...
pub fn purge_expired_trash(conn: &Connection, now: i64) -> DbResult<i64> {
    ...
    conn.execute("DELETE FROM trash WHERE expires_at<=?1 AND restored_at IS NULL", params![now])?;
    Ok(deleted as i64)
}
...
pub fn list_trash(conn: &Connection, volume_id: Option<i64>, limit: i64) -> DbResult<Vec<TrashItem>> {
    if let Some(vid) = volume_id {
        ...
        let rows = stmt.query_map(params![vid, limit], map_trash_item)?.filter_map(|r| r.ok()).collect();
        Ok(rows)
    } else {
        let mut stmt = conn.prepare_cached(
            "SELECT id,entry_id,volume_id,original_path,reason,size_bytes,deleted_at,expires_at
        ...
        let rows = stmt.query_map(params![limit], map_trash_item)?.filter_map(|r| r.ok()).collect();
        Ok(rows)
    }
}
...
fn map_trash_item(row: &Row<'_>) -> rusqlite::Result<TrashItem> {
    Ok(TrashItem {
        id: row.get(0)?, entry_id: row.get(1)?, volume_id: row.get(2)?,
        original_path: row.get(3)?, reason: row.get(4)?, size_bytes: row.get(5)?,
        deleted_at: row.get(6)?, expires_at: row.get(7)?,
    })
}

pub fn restore_trash_entry(conn: &Connection, trash_id: i64, now: i64) -> DbResult<()> {
    // Get entry_id from trash record
    let entry_id: i64 = conn.prepare_cached("SELECT entry_id FROM trash WHERE id=?1")?
        .query_row(params![trash_id], |r| r.get(0))?;
    // Restore entry status
    conn.execute("UPDATE entries SET status='present' WHERE id=?1", params![entry_id])?;
    // Mark trash record as restored
    conn.execute("UPDATE trash SET restored_at=?1 WHERE id=?2", params![now, trash_id])?;
    Ok(())
}
...
pub fn update_volume(conn: &Connection, id: i64, label: &str, disk_number: Option<&str>,
    location_id: Option<i64>, scan_mode: &str) -> DbResult<()>
{
    conn.execute(
...
pub fn update_volume_space(conn: &Connection, id: i64, total: i64, free: i64, used: i64) -> DbResult<()> {
    conn.execute("UPDATE volumes SET total_bytes=?1, free_bytes=?2, used_bytes=?3 WHERE id=?4",
...
pub fn get_volume_kind_stats(conn: &Connection, volume_id: i64) -> DbResult<Vec<KindStat>> {
    ...
    let rows = stmt.query_map(params![volume_id], |r| Ok(KindStat {
        kind: r.get(0)?, count: r.get(1)?, bytes: r.get(2)?,
    }))?.filter_map(|r| r.ok()).collect();
    Ok(rows)
}
...
pub fn get_global_kind_stats(conn: &Connection) -> DbResult<Vec<KindStat>> {
    ...
    let rows = stmt.query_map([], |r| Ok(KindStat {
        kind: r.get(0)?, count: r.get(1)?, bytes: r.get(2)?,
    }))?.filter_map(|r| r.ok()).collect();
    Ok(rows)
}
...
pub fn get_recent_scan_log(conn: &Connection, volume_id: Option<i64>, event_filter: Option<&str>, limit: i64) -> DbResult<Vec<ScanLogEntry>> {
    ...
    let rows: Vec<ScanLogEntry> = match (volume_id, event_filter) {
        (Some(vid), Some(ev)) => stmt.query_map(params![vid, ev, limit], map_scan_log)?.filter_map(|r| r.ok()).collect(),
        (Some(vid), None) => stmt.query_map(params![vid, limit], map_scan_log)?.filter_map(|r| r.ok()).collect(),
        (None, Some(ev)) => stmt.query_map(params![ev, limit], map_scan_log)?.filter_map(|r| r.ok()).collect(),
        (None, None) => stmt.query_map(params![limit], map_scan_log)?.filter_map(|r| r.ok()).collect(),
    };
    Ok(rows)
}
...
fn map_scan_log(row: &Row<'_>) -> rusqlite::Result<ScanLogEntry> {
    Ok(ScanLogEntry {
        id: row.get(0)?, volume_id: row.get(1)?, entry_id: row.get(2)?,
        event: row.get(3)?, new_path: row.get(4)?, old_path: row.get(5)?,
        new_size: row.get(6)?, detected_at: row.get(7)?,
    })
}
...
pub fn get_top_folders(conn: &Connection, volume_id: i64, parent_path: &str, limit: i64) -> DbResult<Vec<FolderStat>> {
    ...
    let mut stmt = conn.prepare_cached(
        "SELECT e.path, e.name, COALESCE(d.bytes_total, 0), COALESCE(d.files_total, 0)
         ...
         LIMIT ?3")?;
    let rows = stmt.query_map(params![volume_id, parent_path, limit], |r| Ok(FolderStat {
        path: r.get(0)?, name: r.get(1)?, bytes_total: r.get(2)?, files_total: r.get(3)?,
    }))?.filter_map(|r| r.ok()).collect();
    Ok(rows)
}
...
pub fn get_volume_snapshots(conn: &Connection, volume_id: i64, limit: i64) -> DbResult<Vec<VolumeSnapshot>> {
    let mut stmt = conn.prepare_cached(
    ...
    let rows = stmt.query_map(params![volume_id, limit], |r| Ok(VolumeSnapshot {
        scanned_at: r.get(0)?, file_count: r.get(1)?, dir_count: r.get(2)?,
        total_bytes: r.get(3)?, used_bytes: r.get(4)?,
    }))?.filter_map(|r| r.ok()).collect();
    Ok(rows)
}
...
pub fn untag_entry(conn: &Connection, entry_id: i64, tag_id: i64) -> DbResult<()> {
    conn.execute("DELETE FROM entry_tags WHERE entry_id=?1 AND tag_id=?2", params![entry_id, tag_id])?;
    Ok(())
}
...
pub struct Collection {
    pub id: i64, pub name: String, pub description: Option<String>,
    pub icon: Option<String>, pub color: Option<String>,
    pub is_smart: bool, pub smart_query: Option<String>,
    pub sort_order: String, pub created_at: i64,
}
...
pub fn list_collections(conn: &Connection) -> DbResult<Vec<Collection>> {
    ...
    let rows = stmt.query_map([], |r| Ok(Collection {
        id: r.get(0)?, name: r.get(1)?, description: r.get(2)?,
        icon: r.get(3)?, color: r.get(4)?, is_smart: r.get::<_,i32>(5)? != 0,
        smart_query: r.get(6)?, sort_order: r.get(7)?, created_at: r.get(8)?,
    }))?.filter_map(|r| r.ok()).collect();
    Ok(rows)
}

pub fn create_collection(conn: &Connection, name: &str, description: Option<&str>, icon: Option<&str>,
    color: Option<&str>, is_smart: bool, smart_query: Option<&str>, now: i64) -> DbResult<i64>
{
    conn.execute(
...
pub fn update_collection(conn: &Connection, id: i64, name: &str, description: Option<&str>,
    icon: Option<&str>, color: Option<&str>, smart_query: Option<&str>, now: i64) -> DbResult<()>
{
    conn.execute(
...
pub fn add_to_collection(conn: &Connection, collection_id: i64, entry_id: i64, now: i64) -> DbResult<()> {
...
pub fn remove_from_collection(conn: &Connection, collection_id: i64, entry_id: i64) -> DbResult<()> {
    conn.execute("DELETE FROM collection_entries WHERE collection_id=?1 AND entry_id=?2",
        params![collection_id, entry_id])?;
    Ok(())
}

pub fn get_collection_entries(conn: &Connection, collection_id: i64, limit: i64) -> DbResult<Vec<EntrySlim>> {
    let mut stmt = conn.prepare_cached(
    ...
    let rows = stmt.query_map(params![collection_id, limit], map_entry_slim)?.filter_map(|r| r.ok()).collect();
    Ok(rows)
}
...
pub struct CustomField {
    pub id: i64, pub name: String, pub field_type: String, pub options: Option<String>, pub sort_order: i64,
}

pub fn list_custom_fields(conn: &Connection) -> DbResult<Vec<CustomField>> {
    let mut stmt = conn.prepare_cached("SELECT id,name,field_type,options,sort_order FROM custom_fields ORDER BY sort_order")?;
    let rows = stmt.query_map([], |r| Ok(CustomField {
        id: r.get(0)?, name: r.get(1)?, field_type: r.get(2)?, options: r.get(3)?, sort_order: r.get(4)?,
    }))?.filter_map(|r| r.ok()).collect();
    Ok(rows)
}

pub fn create_custom_field(conn: &Connection, name: &str, field_type: &str, options: Option<&str>, now: i64) -> DbResult<i64> {
...
pub fn get_entry_custom_values(conn: &Connection, entry_id: i64) -> DbResult<Vec<(i64, String, String, Option<String>)>> {
    let mut stmt = conn.prepare_cached(
    ...
    let rows = stmt.query_map(params![entry_id], |r| Ok((r.get(0)?, r.get(1)?, r.get(2)?, r.get(3)?)))?
        .filter_map(|r| r.ok()).collect();
    Ok(rows)
}

pub fn set_entry_custom_value(conn: &Connection, entry_id: i64, field_id: i64, value: Option<&str>) -> DbResult<()> {
    match value {
        Some(v) => {
            conn.execute("INSERT INTO entry_custom_values (entry_id,field_id,value) VALUES (?1,?2,?3)
                 ON CONFLICT(entry_id,field_id) DO UPDATE SET value=excluded.value",
                params![entry_id, field_id, v])?;
        }
        None => {
            conn.execute("DELETE FROM entry_custom_values WHERE entry_id=?1 AND field_id=?2",
