...
use crate::core::scanner::parallel::{
    self as parallel_scan, ParallelEventCallback, ParallelScanConfig, ParallelScanEvent,
    ParallelScanStats,
};
...
pub struct AppState {
    ...
    pub scan_cancel: Mutex<Option<Sender<()>>>,
    pub hash_cancel: Mutex<Option<Sender<()>>>,
}
...
pub fn list_volumes(state: State<'_, AppState>) -> Result<Vec<Volume>, String> {
    state
        .db
        .read(|c| Ok(queries::list_volumes(c)?))
...
pub fn get_volume(state: State<'_, AppState>, id: i64) -> Result<Option<Volume>, String> {
    state
        .db
        .read(move |c| Ok(queries::get_volume(c, id)?))
        .map_err(|e| e.to_string())
}
...
pub fn add_volume(
    ...
    state
        .db
        .write(move |c| {
            Ok(queries::insert_volume(
                c, &label, &root_path, None, None, now,
            )?)
        })
        .map_err(|e| e.to_string())
}
...
pub fn list_entries(
    ...
    volume_id: i64,
    parent_path: String,
    cursor_is_dir: Option<bool>,
    cursor_mtime: Option<i64>,
    cursor_id: Option<i64>,
    limit: Option<i64>,
) -> Result<Vec<EntrySlim>, String> {
    let cursor = match (cursor_is_dir, cursor_mtime, cursor_id) {
        (Some(d), Some(m), Some(i)) => Some((d, m, i)),
        _ => None,
    };
...
pub fn get_entry(state: State<'_, AppState>, id: i64) -> Result<Option<Entry>, String> {
    state
        .db
        .read(move |c| Ok(queries::get_entry(c, id)?))
        .map_err(|e| e.to_string())
}
...
pub fn search_content(
    ...
    state
        .db
        .read(move |c| Ok(queries::search_text_content(c, &query, lim)?))
        .map_err(|e| e.to_string())
}
...
pub async fn start_scan(
    ...
    let volume = state
        .db
        .read(move |c| Ok(queries::get_volume(c, volume_id)?))
        .map_err(|e| e.to_string())?
    ...
    if let Some(h) = compute_hash {
        config.compute_hash = h;
    }
    if let Some(t) = generate_thumbs {
        config.generate_thumbs = t;
    }

    let db = state.db.clone();
    let (cancel_tx, cancel_rx) = bounded(1);
    {
        let mut guard = state.scan_cancel.lock().unwrap();
        if guard.is_some() {
    ...
    state.scan_cancel.lock().unwrap().take();
    result
}

#[command]
pub fn cancel_scan(state: State<'_, AppState>) -> Result<bool, String> {
    let tx = state.scan_cancel.lock().unwrap().clone();
    if let Some(cancel_tx) = tx {
        Ok(cancel_tx.send(()).is_ok())
    } else {
        Ok(false)
    }
}
...
pub fn list_active_jobs(state: State<'_, AppState>) -> Result<Vec<Job>, String> {
    state
        .db
        .read(|c| Ok(queries::list_active_jobs(c)?))
        .map_err(|e| e.to_string())
}
...
pub fn find_duplicates(
    ...
    state
        .db
        .read(move |c| Ok(queries::find_duplicates(c, min_size)?))
        .map_err(|e| e.to_string())
}
...
pub fn get_duplicate_group(state: State<'_, AppState>, hash: String) -> Result<Vec<Entry>, String> {
    state
        .db
        .read(move |c| Ok(queries::get_duplicate_group(c, &hash)?))
        .map_err(|e| e.to_string())
}
...
pub fn get_setting(state: State<'_, AppState>, key: String) -> Result<Option<String>, String> {
    state
        .db
        .read(move |c| Ok(queries::get_setting(c, &key)?))
        .map_err(|e| e.to_string())
}
...
pub fn set_setting(state: State<'_, AppState>, key: String, value: String) -> Result<(), String> {
    ...
    state
        .db
        .write(move |c| Ok(queries::set_setting(c, &key, &value, now)?))
        .map_err(|e| e.to_string())
}
...
pub fn create_tag(
    ...
    state
        .db
        .write(move |c| Ok(queries::create_tag(c, &name, color.as_deref(), now)?))
        .map_err(|e| e.to_string())
}
...
pub fn tag_entry(state: State<'_, AppState>, entry_id: i64, tag_id: i64) -> Result<(), String> {
    state
        .db
        .write(move |c| Ok(queries::tag_entry(c, entry_id, tag_id)?))
        .map_err(|e| e.to_string())
}
...
pub fn get_entry_tags(
    ...
    state
        .db
        .read(move |c| Ok(queries::get_entry_tags(c, entry_id)?))
        .map_err(|e| e.to_string())
}
...
pub fn get_entry_tags_bulk(
    ...
    state
        .db
        .read(move |c| Ok(queries::get_entry_tags_bulk(c, &entry_ids)?))
        .map_err(|e| e.to_string())
}
    ...
    state
        ...
        .write(move |conn| {
            ...
            let retention: i64 = queries::get_setting(conn, "trash.retention_days")?
                ...
                .unwrap_or(30);
            Ok(queries::trash_entry(
                conn,
                entry_id,
                entry.volume_id,
                &entry.path,
                entry.full_hash.as_deref(),
                &r,
                entry.size_bytes,
                None,
                now,
...
pub async fn start_hash(
    ...
    let (cancel_tx, cancel_rx) = bounded(1);
    {
        let mut guard = state.hash_cancel.lock().unwrap();
        if guard.is_some() {
            return Err("A hash job is already running".into());
        }
        *guard = Some(cancel_tx.clone());
    }
    let app_handle = app.clone();
    ...
    state.hash_cancel.lock().unwrap().take();
    result
}

#[command]
pub fn cancel_hash(state: State<'_, AppState>) -> Result<bool, String> {
    let tx = state.hash_cancel.lock().unwrap().clone();
    if let Some(cancel_tx) = tx {
        Ok(cancel_tx.send(()).is_ok())
    } else {
        Ok(false)
...
pub fn update_volume(
    ...
    state
        .db
        .write(move |c| {
            Ok(queries::update_volume(
                c,
                id,
                &label,
                disk_number.as_deref(),
                location_id,
                &scan_mode,
            )?)
        })
...
pub fn update_volume_space(
    ...
    state
        .db
        .write(move |c| Ok(queries::update_volume_space(c, id, total, free, used)?))
        .map_err(|e| e.to_string())
}
...
pub fn delete_volume(state: State<'_, AppState>, id: i64) -> Result<(), String> {
    state
        .db
        .write(move |c| Ok(queries::delete_volume(c, id)?))
        .map_err(|e| e.to_string())
}
...
pub fn list_trash(
    ...
    volume_id: Option<i64>,
    limit: Option<i64>,
) -> Result<Vec<queries::TrashItem>, String> {
    let lim = limit.unwrap_or(100);
    state
        .db
        .read(move |c| Ok(queries::list_trash(c, volume_id, lim)?))
        .map_err(|e| e.to_string())
}
...
pub fn restore_entry(state: State<'_, AppState>, trash_id: i64) -> Result<(), String> {
    ...
    state
        .db
        .write(move |c| Ok(queries::restore_trash_entry(c, trash_id, now)?))
        .map_err(|e| e.to_string())
}
...
pub fn purge_expired(state: State<'_, AppState>) -> Result<i64, String> {
    ...
    state
        .db
        .write(move |c| Ok(queries::purge_expired_trash(c, now)?))
        .map_err(|e| e.to_string())
}
...
pub fn trash_summary(state: State<'_, AppState>) -> Result<(i64, i64), String> {
    state
        .db
        .read(|c| Ok(queries::trash_summary(c)?))
        .map_err(|e| e.to_string())
}
...
#[command]
pub fn get_volume_kind_stats(
    state: State<'_, AppState>,
    volume_id: i64,
) -> Result<Vec<queries::KindStat>, String> {
    state
        .db
        .read(move |c| Ok(queries::get_volume_kind_stats(c, volume_id)?))
        .map_err(|e| e.to_string())
}
...
pub fn get_global_kind_stats(state: State<'_, AppState>) -> Result<Vec<queries::KindStat>, String> {
    state
        .db
        .read(|c| Ok(queries::get_global_kind_stats(c)?))
        .map_err(|e| e.to_string())
}
...
pub fn get_recent_scan_log(
    ...
    state
        .db
        .read(move |c| {
            Ok(queries::get_recent_scan_log(
                c,
                volume_id,
...
pub fn get_top_folders(
    ...
    parent_path: String,
    limit: Option<i64>,
) -> Result<Vec<queries::FolderStat>, String> {
    let lim = limit.unwrap_or(20);
    state
        .db
        .read(move |c| Ok(queries::get_top_folders(c, volume_id, &parent_path, lim)?))
        .map_err(|e| e.to_string())
}
...
pub fn get_volume_snapshots(
    ...
    volume_id: i64,
    limit: Option<i64>,
) -> Result<Vec<queries::VolumeSnapshot>, String> {
    let lim = limit.unwrap_or(30);
    state
        .db
        .read(move |c| Ok(queries::get_volume_snapshots(c, volume_id, lim)?))
        .map_err(|e| e.to_string())
}
...
pub fn update_tag(
    ...
    state
        .db
        .write(move |c| Ok(queries::update_tag(c, id, &name, color.as_deref())?))
        .map_err(|e| e.to_string())
}
...
pub fn delete_tag(state: State<'_, AppState>, id: i64) -> Result<(), String> {
    state
        .db
        .write(move |c| Ok(queries::delete_tag(c, id)?))
        .map_err(|e| e.to_string())
}
...
pub fn untag_entry(state: State<'_, AppState>, entry_id: i64, tag_id: i64) -> Result<(), String> {
    state
        .db
        .write(move |c| Ok(queries::untag_entry(c, entry_id, tag_id)?))
        .map_err(|e| e.to_string())
}
...
pub fn list_collections(state: State<'_, AppState>) -> Result<Vec<queries::Collection>, String> {
    state
        .db
        .read(|c| Ok(queries::list_collections(c)?))
        .map_err(|e| e.to_string())
}
...
pub fn delete_collection(state: State<'_, AppState>, id: i64) -> Result<(), String> {
    state
        .db
        .write(move |c| Ok(queries::delete_collection(c, id)?))
        .map_err(|e| e.to_string())
}
...
pub fn add_to_collection(
    ...
    state
        .db
        .write(move |c| Ok(queries::add_to_collection(c, collection_id, entry_id, now)?))
        .map_err(|e| e.to_string())
}
...
pub fn remove_from_collection(
    ...
    state
        .db
        .write(move |c| Ok(queries::remove_from_collection(c, collection_id, entry_id)?))
        .map_err(|e| e.to_string())
}
...
pub fn get_collection_entries(
    ...
    state
        .db
        .read(move |c| Ok(queries::get_collection_entries(c, collection_id, lim)?))
        .map_err(|e| e.to_string())
}
...
pub fn list_custom_fields(state: State<'_, AppState>) -> Result<Vec<queries::CustomField>, String> {
    state
        .db
        .read(|c| Ok(queries::list_custom_fields(c)?))
        .map_err(|e| e.to_string())
}
...
pub fn create_custom_field(
    ...
    state
        .db
        .write(move |c| {
            Ok(queries::create_custom_field(
                c,
                &name,
                &field_type,
                options.as_deref(),
...
pub fn delete_custom_field(state: State<'_, AppState>, id: i64) -> Result<(), String> {
    state
        .db
        .write(move |c| Ok(queries::delete_custom_field(c, id)?))
        .map_err(|e| e.to_string())
}
...
pub fn get_entry_custom_values(
    ...
    state
        .db
        .read(move |c| Ok(queries::get_entry_custom_values(c, entry_id)?))
        .map_err(|e| e.to_string())
}
...
pub fn set_entry_custom_value(
    ...
    state
        .db
        .write(move |c| {
            Ok(queries::set_entry_custom_value(
                c,
                entry_id,
                field_id,
                value.as_deref(),
...
pub fn preview_rename(
    ...
    entry_ids: Vec<i64>,
    pattern: String,
) -> Result<Vec<rename::RenamePreview>, String> {
...
pub fn apply_rename(
    state: State<'_, AppState>,
    previews: Vec<rename::RenamePreview>,
    volume_id: i64,
) -> Result<rename::RenameStats, String> {
...
pub fn ai_classify(
    state: State<'_, AppState>,
    entry_id: i64,
) -> Result<ai::ClassifyResult, String> {
    ...
    let text = state
        ...
        .map_err(|e| e.to_string())?
        .ok_or("No text extracted for this entry")?;
    ...
    let path: String = state
        ...
        .read(move |c| {
            c.prepare_cached("SELECT path FROM entries WHERE id=?1")?
                .query_row(params![entry_id], |r| r.get(0))
                .map_err(crate::db::DbError::Sqlite)
        })
        .map_err(|e| e.to_string())?;
    ai::analyze_image(&state.db, &config, entry_id, std::path::Path::new(&path))
        .map_err(|e| e.to_string())
}
...
pub async fn start_parallel_scan(
    ...
    let app_handle = app.clone();
    let on_event: Option<ParallelEventCallback> =
        Some(std::sync::Arc::new(move |evt: ParallelScanEvent| {
...
pub fn start_watch(
    ...
    let watcher =
        FsWatcher::start(state.db.clone(), config, on_event).map_err(|e| e.to_string())?;
...
#[command]
pub fn get_db_diagnostics(
    state: State<'_, AppState>,
) -> Result<crate::db::pragmas::PragmaDiagnostics, String> {
    state
        .db
